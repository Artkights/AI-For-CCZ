...
/// <summary>
/// R/S eex 命令引用候选核对清单。
/// 把结构草图里的“可跳转引用候选”导出为中文 Markdown，供创作者逐项核对人物、物品、策略、文本、地图/坐标与备注。
...
public sealed class ScenarioCommandReferenceChecklistService
{
        ...
        IReadOnlyList<CreatorNote>? creatorNotes = null)
    {
        ...
        File.WriteAllText(reportPath, BuildReport(project, tables, structure, rows, mapLink, textEntries, creatorNotes), Encoding.UTF8);
        return reportPath;
    }
        ...
        IReadOnlyList<CreatorNote>? creatorNotes = null)
    {
        ...
        var allCommandRows = structure.Rows.Where(row => row.NodeType == "Command候选").ToList();
        var noteLookup = BuildNoteLookup(creatorNotes ?? Array.Empty<CreatorNote>());
        var commandTargets = BuildCommandTargets(project, tables, structure, commandRows, mapLink, textEntries);
        ...
        AppendPriorityChecklist(builder, commandTargets, noteLookup);
        AppendCommandDetails(builder, commandTargets, noteLookup);
        ...
        IReadOnlyList<ScenarioTextEntry> textEntries)
    {
        ...
        {
            var targets = _referenceNavigationService.Analyze(project, tables, row, structure.FileName, mapLink, textEntries);
    ...
    private static void AppendOverview(StringBuilder builder, IReadOnlyList<ScenarioStructureRow> rows, IReadOnlyList<CommandTargets> commandTargets)
    {
        ...
        builder.AppendLine($"| 有地图/坐标候选的命令 | {commandsWithMap} | 可跳到关卡地图联动核对底图、地形和坐标。 |");
        builder.AppendLine();
    ...
    private static void AppendPriorityChecklist(StringBuilder builder, IReadOnlyList<CommandTargets> commandTargets, IReadOnlyDictionary<string, int> noteLookup)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("| 命令 | 候选 | 可跳转 | 备注 | 核对建议 |");
        builder.AppendLine("| --- | --- | --- | ---: | --- |");
        if (rows.Count == 0)
        {
        ...
        foreach (var item in rows)
        {
            ...
            var jumpText = string.Join(" / ", item.Targets
                ...
                .DefaultIfEmpty("-"));
            var noteCount = noteLookup.TryGetValue(BuildScenarioCommandCreatorNoteTargetKey(string.Empty, item.Row), out var count) ? count : 0;
            builder.AppendLine(
    ...
    private static void AppendCommandDetails(StringBuilder builder, IReadOnlyList<CommandTargets> commandTargets, IReadOnlyDictionary<string, int> noteLookup)
    {
        ...
        foreach (var item in commandTargets.Where(item => item.Targets.Count > 0).Take(120))
        {
            ...
            builder.AppendLine($"- 创作者备注：{noteCount} 条");
            builder.AppendLine();
    ...
    private static IReadOnlyDictionary<string, int> BuildNoteLookup(IEnumerable<CreatorNote> notes)
    {
        var result = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        foreach (var note in notes.Where(note =>
                     note.Scope.Equals("R/S命令", StringComparison.OrdinalIgnoreCase) ||
                     note.Scope.Equals("SV命令", StringComparison.OrdinalIgnoreCase)))
        {
            var normalized = NormalizeScenarioCommandTargetKey(note.TargetKey);
            result.TryGetValue(normalized, out var count);
            result[normalized] = count + 1;
        }
    ...
    private static string BuildScenarioCommandCreatorNoteTargetKey(string fileName, ScenarioStructureRow row)
        => NormalizeScenarioCommandTargetKey($"{fileName}#Scene={row.SceneIndex}#Section={row.SectionIndex}#Command={row.CommandIndex}#Offset={row.OffsetHex}");

    private static string NormalizeScenarioCommandTargetKey(string targetKey)
    {
        var index = targetKey.IndexOf("#Scene=", StringComparison.OrdinalIgnoreCase);
        return index >= 0 ? targetKey[index..] : targetKey;
    }
    ...
    private static IEnumerable<string> BuildJumpKinds(ScenarioCommandReferenceTarget target)
    {
        ...
        if (target.CanJumpScenarioMap) yield return "地图联动";
    }
    ...
    private static string BuildKindSuggestion(string kind) => kind switch
    {
        ...
        "地图" or "坐标" => "跳到关卡地图联动核对底图、地形块和坐标范围。",
