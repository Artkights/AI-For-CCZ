...
- Capability discovery: `read_mcp_capability_manifest` is the authoritative runtime manifest. It reports the service name, build/document version, tool count, groups, aliases, removed tools, safety notes, and the full sorted tool list. Use it when a client UI truncates `tools/list`; validation requires the manifest count to match `tools/list`. The 2026-07-10 validation result is 220 authoring tools, but clients should rely on the runtime manifest instead of a hard-coded count.
...
- Game settings data: `list_tables`, `read_table`, `write_table_rows`, `read_table_schema`, `read_table_derived_display`, `export_table_csv`, `preview_import_table_csv`, `apply_import_table_csv`, `read_job_settings`, `preview_job_settings`, `write_job_settings`, `read_accessory_job_groups`, `preview_accessory_job_groups`, `write_accessory_job_groups`, `list_effects`, `read_effect`, `export_effect_package`, `preview_effect_package`, `apply_effect_package`, `list_effect_templates`, `build_effect_package_from_template`, `preview_effect_patch`, `apply_effect_patch`, `scan_installed_effects`, `read_effect_instance`, `explain_exe_address`, `read_effect_generation_context`, `preview_effect_parameter_update`, `scan_exe_code_caves`, `draft_assembly_patch`, `preview_assembly_patch`, `apply_assembly_patch`, `draft_special_skill_patch`, `preview_special_skill_patch`, `apply_special_skill_patch`, `rebind_special_skill_params`, `read_effect_resource`, `read_effect_prompt`. For local-agent generated injectable special effects, read the compact generation context and Hook contracts, generate `InlineSpecialSkillPatchDraft` or `AssemblyPatchDraft` outside the tool, preview it, then apply only the compiled package returned by the preview tool.
