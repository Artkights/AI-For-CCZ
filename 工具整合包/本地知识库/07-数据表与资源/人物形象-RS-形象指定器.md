...
1. 人物 R/S 指定数据不应凭 UI 表名待证假设，必须按 B形象指定器的 `System.ini` 校对。
2. 6.5 版本：
...
- 6.6x 的人物 R/S 指定表偏移仍与 6.5 一致：R=`0xE1000`，S=`0xD2800`，每项仍可按 UInt16 风格读取。
- 用 `Ekd5 6.6.exe` 直接读取：`0xE1000` 前 32 个 R 值为 `0,1,2,3,4,5,7,6,0...`；`0xD2800` 前 32 个 S 值均为 `0`。这说明 6.6x 引擎样本在 R/S 指定段存在可读数据，且 6.5 内置 R/S 表偏移可作为临时只读兜底。
...
#### 2026-06-07 AI 头像/R/S 草稿

- `AI绘图-Image-Studio-MCP.md` 已新增 `face`、`r_actor`、`s_unit` preset，用于从自然语言描述生成头像、R 形象和 S 形象草稿。
...
- 该 AI 流程只做导出和替换预览；完整动作帧、朝向、帧序和实机表现仍是待验证项，不能把 v1 静态条带当作完整形象制作器。
...
- `兵种名修改`：
  - 使用 `6.5-3 兵种系`。
  - 目标：`Ekd5.exe @ 0x8B010`，`40 x 9B`。
  - 证据：`HexTable.xml`、本地样本、现有兵种系写回 smoke。
- `职业名修改`：
  - 使用 `6.5-4 详细兵种`。
  - 目标：`Ekd5.exe @ 0xD18D0`，`80 x 9B`。
  - 证据：`HexTable.xml`、本地样本、现有详细兵种写回 smoke。
- `游戏标题修改`：
  - 当前 6.5 样本 `Ekd5.exe` 中 GBK 文本 `【三国志曹操传 加强版】` 位于 `0x8D3C4`。
