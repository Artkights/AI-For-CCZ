...
- `replace_e5_image_entry` 只允许可读取 `0x110` 索引表的 `.e5` 图片载荷文件，且拒绝 `Data.e5`、`Imsg.e5`、`Star.e5`、`Hexzmap.e5` 等核心文件；人物 R/S 指定编号仍必须通过表格写入，不能用 E5 图片替换工具改核心表。`Mark.e5` 当前样本不是标准 `0x110` 索引封包，只定位不替换；`Itemicon.dll / Mgcicon.dll / Cmdicon.dll` 属于 DLL 资源，只读预览，不通过该工具写回。
