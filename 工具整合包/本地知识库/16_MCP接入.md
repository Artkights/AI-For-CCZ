...
- 2026-06-04：使用 SDK `StdioClientTransport` 连接本地 MCP Server，当前 `tools/list` 返回 28 个 tools，并确认 E5 图片条目、项目体检、测试副本、差异、发布闭环、知识库搜索、剧本指令模板、R/S 剧本结构只读工具、Hexzmap 读取工具、资源索引和资源诊断工具均可见；调用 `list_e5_image_entries target_relative_path=Unit_mov.e5 limit=3` 成功返回 `totalEntries=556`。
