...
if (-not $NoBuild -and -not (Test-ServerOutputCurrent $serverOutput)) {
    $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ccz-game-debug-mcp-server-" + [guid]::NewGuid().ToString("N"))
