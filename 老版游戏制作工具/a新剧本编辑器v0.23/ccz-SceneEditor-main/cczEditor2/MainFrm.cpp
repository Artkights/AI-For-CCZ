...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWndEx)
	...
	ON_WM_CONTEXTMENU()
	ON_WM_SETTINGCHANGE()
	ON_COMMAND(ID_ALL_FILE_CLOSE, &CMainFrame::OnAllFileClose)
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	/*m_icon = LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCEW(IDI_BIRD));
	SetClassLong(m_hWnd, GCL_HICON, (LONG)m_icon);*/
		...
		if (IsMemberOfMDITabGroup(pWndCur))
		{
			...
			if (pWndTab != NULL)
			{
				...
				if (nTab >= 0)
				{
					...
					CMenu menu;
					CMenu* pSubMenu = NULL;
					menu.LoadMenu(IDR_cczEditor2TYPE2);
...
void CMainFrame::OnAllFileClose()
{
	...
	while (pos != NULL)
	{
		...
		while (posdoc != NULL)
		{
			...
			if (pDoc != NULL)
			{
				((CcczEditor2Doc*)pDoc)->SaveModified();
				if (((CcczEditor2Doc*)pDoc)->can_close || !((CcczEditor2Doc*)pDoc)->modified)
					((CcczEditor2Doc*)pDoc)->OnCloseDocument();
			}
		}
	}
}
